...
export const HEAL_SCALING_DEFS: HealScalingDef[] = [
  ...
  // Draconic Rune Infusion - Dragon Claw heal
  {
    sourceName: 'Dragon Claw (Holy)',
    sourceType: 'weaponArt',
    appliesTo: ['weaponArt'],
    calcFn: (ctx) => {
      const perkAmount = ctx.perks['Draconic Blood'] ?? 0
      if (perkAmount > 0 && ctx.draconicColor === 'holy') {
        const healValue = 5 + 0.5 * perkAmount
        return {
          multiplier: healValue,
          condition: `${healValue.toFixed(1)} heal per cast`,
        }
      }
      return null
    },
  ...
  {
    sourceName: 'Dragon Claw (Water)',
    sourceType: 'weaponArt',
    appliesTo: ['weaponArt'],
    calcFn: (ctx) => {
      const perkAmount = ctx.perks['Draconic Blood'] ?? 0
      if (perkAmount > 0 && ctx.draconicColor === 'water') {
        const healValue = Math.round((1.923 + 0.1923 * perkAmount) * 1000) / 1000
        return {
          multiplier: healValue,
          condition: `${healValue.toFixed(3)} heal per cast`,
        }
      }
      return null
    },
  ...
  // Draconic Rune Infusion - Dragon Bubble heal
  {
    sourceName: 'Dragon Bubble (Holy)',
    sourceType: 'weaponArt',
    appliesTo: ['weaponArt'],
    calcFn: (ctx) => {
      const perkAmount = ctx.perks['Draconic Blood'] ?? 0
      if (perkAmount > 0 && ctx.draconicColor === 'holy') {
        const healValue = 4 + 0.4 * perkAmount
        return {
          multiplier: healValue,
          condition: `${healValue.toFixed(1)} heal per cast`,
        }
  ...
  {
    sourceName: 'Dragon Bubble (Water)',
    sourceType: 'weaponArt',
    appliesTo: ['weaponArt'],
    calcFn: (ctx) => {
      const perkAmount = ctx.perks['Draconic Blood'] ?? 0
      if (perkAmount > 0 && ctx.draconicColor === 'water') {
        const healValue = Math.round((1.538 + 0.1538 * perkAmount) * 1000) / 1000
        return {
          multiplier: healValue,
          condition: `${healValue.toFixed(3)} heal per cast`,
        }
