...
try {
  ...
  graph.layers.forEach(layer => {
    (layer.nodeIds || []).forEach(id => {
      if (!nodeIds.has(id)) issues.push('Layer refs missing node ' + id);
  ...
  fs.writeFileSync(outputPath, JSON.stringify({ issues: issues.slice(0, 50), warnings: warnings.slice(0, 50), stats }, null, 2));
